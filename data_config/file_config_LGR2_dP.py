...
for names in to_load_names:
    ...
    for well in WELL_LIST:
        gridx = np.repeat(xy_norm(GRID_CENTER_DICT[well]['LGR2']['grid_x'])[...,None,None], 24, axis=-2)
        gridy = np.repeat(xy_norm(GRID_CENTER_DICT[well]['LGR2']['grid_y'])[...,None,None], 24, axis=-2)
        gridz = np.repeat(z_norm(TOPS_DICT[well]['LGR2'][0,...,None,None]), 24, axis=-2)
        gridt = (np.ones(gridz.shape)* times[None,None,None,:,None])
        
        inj = np.repeat(INJ_MAP_DICT[well]['LGR2'][...,None,None], 24, axis=-2)
        pressure = np.repeat(p_norm(return_upsample_dict(OUTPUT_DICT, 0, 'BPR', 
                                               WELL_LIST, GRID_IDX_DICT)[well]['LGR2'][0,...,None,None]), 24, axis=-2)
        temp = t_norm(t) * np.ones(inj.shape)
        perm = np.repeat(k_norm(PERM_DICT[well]['LGR2'])[0,...,None,None], 24, axis=-2)

        
        DICT = return_upsample_all_time(OUTPUT_DICT, 'dP', WELL_LIST, 
                                        GRID_IDX_DICT, LGR_LIST)
        
        coarse = np.repeat(DICT[well]['LGR1'][0,:,:,:,:,None],2,axis=2)
        x_DP = np.concatenate([gridx, gridy, gridz, gridt, inj, pressure, temp, perm, coarse], axis=-1)[None,...]
        y_DP = DICT[well]['LGR2'][...,None]
        ...
        data['input'] = x_DP
        data['output'] = y_DP
        print(f'{slope_name}_{idx}_LGR2_{well}_DP.pt')

        torch.save(data, f'../dataset/dP_LGR2/{slope_name}_{idx}_LGR2_{well}_DP.pt')