...
for names in to_load_names:
    ...
    for well in WELL_LIST:
        ...
        pressure = np.repeat(p_norm(return_upsample_dict(OUTPUT_DICT, 0, 'BPR', 
                                               WELL_LIST, GRID_IDX_DICT)[well]['LGR1'][0,...,None,None]), 24, axis=-2)
        ...
        coarse = np.repeat(DICT['GLOBAL'][0,I1:I2,J1:J2,:,:,None],5,axis=2)
        ...
        data['input'] = x_DP
        data['output'] = y_DP
        print(f'{slope_name}_{idx}_LGR1_{well}_SG.pt')
        
        torch.save(data, f'../dataset/SG_LGR1/{slope_name}_{idx}_LGR1_{well}_SG.pt')